...
const TopNav = () => {
  ...
  return (
    <TopNavContainer>
      <AppBar component="nav" sx={{ backgroundColor: theme.palette.secondary.main, color: theme.white.main }}>
        <Toolbar>
          <IconButton
            color="inherit"
          ...
          <Box sx={{ display: { xs: 'none', sm: 'block' } }}>
            {navItems.map((item) => (
              <Button key={item.id}>
                <Link href={item.path} style={{ color: theme.white.main }}>{item.title}</Link>
              </Button>
            ))}
          </Box>
        </Toolbar>
